    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if not request.session.test_cookie_worked():
                error_message = u'wrong cookie, please try again'
            else:
                ...
                if error_code == 0:
                    ...
                    error_message = u'none existing user, please try again'
                elif error_code == 2:
                    error_message = u'password incorrect, please try again'
        else:
            error_message = u'incorrect form, please try again'
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if user.password == form.cleaned_data['cur_password']:
                ...
                error_message = 'original password incorrect'
        else:
            error_message = form.errors
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is None or len(user) == 0:
                error_message = u'none existing email'
            else:
                user = user[0]
...
def reset_password(request, token_str):
    ...
    try:
        ...
        if TimestampGenerator(0)() > token.expire:
            error_message = u'token expired, please try again'
    except ResetPasswordRequest.DoesNotExist:
        error_message = u'wrong link, please try again'
